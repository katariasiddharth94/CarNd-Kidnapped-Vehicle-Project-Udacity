...
void ParticleFilter::prediction(double delta_t, double std_pos[], 
  ...
  // calculate new x, y, theta for all particles
...
void ParticleFilter::updateWeights(double sensor_range, double std_landmark[], 
  ...
  for(unsigned i = 0; i < particles.size(); ++i){
    ...
    for(unsigned j = 0; j < observations.size(); j++){
      ...
      new_obs.x = particles[i].x + (cos(particles[i].theta) * observations[j].x) - (sin(particles[i].theta) * observations[j].y);
      new_obs.y = particles[i].y + (sin(particles[i].theta) * observations[j].x) + (cos(particles[i].theta) * observations[j].y);
      
      // find nearest landmark
      double nearest_landmark_distance = sensor_range;
      int nearest_landmark = 0; // landmarks range from 1-42
      for(unsigned k = 0; k < map_landmarks.landmark_list.size(); ++k){
      	double landmark_distance = dist(new_obs.x, new_obs.y, map_landmarks.landmark_list[k].x_f, map_landmarks.landmark_list[k].y_f);
      	if(landmark_distance < nearest_landmark_distance){
        	nearest_landmark_distance = landmark_distance;
        	nearest_landmark = k + 1; // 1 based
      	}
      }
...
void ParticleFilter::resample() {
   ...
   */
  // I referred to the Udacity Q & A session to understand how to implement this method
